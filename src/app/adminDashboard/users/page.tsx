...
const UsersPage: React.FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch(
        "https://recipebackend-phi.vercel.app/users",
        {
          method: "GET",
          headers: {
  ...
  const blockUser = async (userId: string) => {
    try {
      const token = localStorage.getItem("token");
      await fetch(`https://recipebackend-phi.vercel.app/block/${userId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  const unblockUser = async (userId: string) => {
    try {
      const token = localStorage.getItem("token");
      await fetch(`https://recipebackend-phi.vercel.app/unblock/${userId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`, // Corrected template literal
  ...
  const deleteUser = async (userId: string) => {
    try {
      const token = localStorage.getItem("token");
      await fetch(`https://recipebackend-phi.vercel.app/delete/${userId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`, // Corrected template literal
            ...
            <table className="min-w-full table-auto">
              ...
              <tbody>
                {users.map((user) => (
                  <tr
                    key={user._id}
                    className="bg-white border-b hover:bg-gray-100 transition duration-300"
                  >
